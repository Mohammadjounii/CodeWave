...
namespace CodeWave.Infrastructure.Data.Seed
{
    public static class JobOfferSeed
    {
        public static async Task SeedAsync(ApplicationDbContext context)
        {
            ...
            // Update RequiredLevel on existing seed jobs (idempotent)
            var levelMap = jobs.ToDictionary(j => j.Id, j => j.RequiredLevel);
            var existingToUpdate = await context.JobOffers
                .Where(j => levelMap.Keys.Contains(j.Id))
                .ToListAsync();
            foreach (var job in existingToUpdate)
            {
                if (levelMap.TryGetValue(job.Id, out var level) && job.RequiredLevel != level)
                    job.RequiredLevel = level;
            }
            await context.SaveChangesAsync();
        }
    }
