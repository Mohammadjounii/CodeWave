...
namespace CodeWave.Web.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        {
            // Called via fetch() from the shared profile-picture modal (see _UserSidebarContent.cshtml).
            // Returns JSON — that modal is rendered on every page (Playground, Projects, Cv, ...),
            // so redirecting here would always bounce the user back to the Home dashboard.
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            if (string.IsNullOrEmpty(userId) || !Guid.TryParse(userId, out var userGuid))
            {
                return Unauthorized(new { success = false, message = "You need to be logged in." });
            }
            ...
            {
                return BadRequest(new { success = false, message = "File size too large. Please upload an image smaller than 5MB." });
            }
            ...
            try
            {
                ...
                // Best-effort cleanup of the old picture — a locked/in-use old file (e.g. still
                // being served to a browser tab) must never block saving the new one.
                var previousPictureUrl = user.ProfilePictureUrl;
                if (!string.IsNullOrEmpty(previousPictureUrl))
                {
                    try
                    {
                        var oldFilePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", previousPictureUrl.TrimStart('/'));
                    ...
                    {
                        // Ignore — orphaned old file is harmless, failing the upload over it is not.
                    }
                }
                ...
                return Ok(new { success = true, url = user.ProfilePictureUrl, message = "Profile picture updated successfully!" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, new { success = false, message = $"Error uploading picture: {ex.Message}" });
            }
        }
