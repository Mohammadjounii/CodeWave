...
namespace CodeWave.Web.Controllers
{
    ...
    public class ProjectsController : Controller
    {
        ...
        public async Task<IActionResult> Save(Project model)
        {
            ...
            var earliestAllowedDate = today.AddYears(-50);
            if (model.CompletionDate.Date > today)
            {
                ModelState.AddModelError(nameof(model.CompletionDate), "Completion date can't be in the future.");
            }
            else if (model.CompletionDate.Date < earliestAllowedDate)
            {
                ModelState.AddModelError(nameof(model.CompletionDate), $"Completion date must be after {earliestAllowedDate:MMMM yyyy}.");
            }
