...
public class CodeService : ICodeService
{
    ...
    public async Task<SubmitExerciseResultDto> RunAndSaveAsync(RunCodeRequestDto request, Guid userId)
    {
        ...
        if (!request.ExerciseId.HasValue)
        {
            ...
            try
            {
                // Playground code runs exactly as written — no test-case wiring here,
                // so we must not auto-invoke whatever function happens to be defined first.
                string result;
                if (detectedLanguage.Equals("python", StringComparison.OrdinalIgnoreCase))
                {
                    result = await _runner.RunPythonAsync(request.Code);
                }
                else if (detectedLanguage.Equals("java", StringComparison.OrdinalIgnoreCase))
                {
                    result = await _runner.RunJavaAsync(request.Code);
                }
                else
                {
                    return new SubmitExerciseResultDto
        ...
        if (testCases.Any())
        {
            foreach (var testCase in testCases)
            {
                ...
                try
                {
                    ...
                    else if (detectedLanguage.Equals("javascript", StringComparison.OrdinalIgnoreCase))
                    {
                        result = await _runner.RunJavaScriptAsync(codeToRun);
                    }
                    else
                    {
                        return new SubmitExerciseResultDto
    ...
    private string DetectLanguageFromCourse(Course course)
    {
        ...
        {
            return course.ProgrammingLanguage.Value switch
            {
                Language.Python => "python",
                Language.Java => "java",
                Language.WebDevelopment => "javascript",
                _ => "python"
            };
        }
        ...
        if (learningPath.Contains("python")) return "python";
        if (learningPath.Contains("web")) return "javascript";
        if (learningPath.Contains("java")) return "java";
