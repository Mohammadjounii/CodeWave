    ...
    public class CvController : Controller
    {
            ...
            string template = "dublin")
        {
            ...
            if (string.IsNullOrEmpty(TempData["Error"] as string))
            {
                ...
                var _achSvc = HttpContext.RequestServices.GetService<IAchievementService>();
                if (_achSvc != null && Guid.TryParse(userId, out var achUserId))
                    await _achSvc.TryAwardAsync(achUserId, "cv_created");
            }
        ...
        public async Task<IActionResult> UpgradeCV(string template = "dublin")
        {
            ...
            if (cv == null)
            {
                TempData["Error"] = "No CV data found. Please build your CV first in the Build CV section.";
                TempData["ActiveTab"] = "upgrade";
                return RedirectToAction("Index");
            }
            ...
            try
            {
                // Apply the selected template and regenerate the PDF from CV Builder data
                cv.Template = !string.IsNullOrEmpty(template) ? template : "dublin";
                cv.LastUpdated = DateTime.UtcNow;
                await _cvService.CreateOrUpdateCVAsync(cv);
                ...
                cv.GeneratedPDFPath = $"/uploads/cv-pdfs/{pdfFileName}";
                await _cvService.CreateOrUpdateCVAsync(cv);

                TempData["Success"] = $"CV regenerated with the {cv.Template} template! Download your new PDF below.";
            }
            catch (Exception ex)
            {
                TempData["Error"] = $"Error generating CV: {ex.Message}";
            }
        ...
        public async Task<IActionResult> AutoFillCV()
        {
            ...
            // Generate Projects from User Projects (serialized as JSON to match CV form structure)
            var userProjects = await _projectRepository.GetUserProjectsAsync(userGuid);

            if (userProjects.Any())
            {
                var projectsList = userProjects.Select(p => new
                {
                    name        = p.Title ?? string.Empty,
                    description = p.Description ?? string.Empty,
                    technologies = !string.IsNullOrEmpty(p.Result)
                        ? p.Result.Split(',').Select(t => t.Trim()).Where(t => t.Length > 0).ToArray()
                        : Array.Empty<string>(),
                    link = string.Empty,
                    date = p.CompletionDate != default
                        ? p.CompletionDate.ToString("yyyy-MM")
                        : string.Empty
                });
                cv.Projects = JsonSerializer.Serialize(projectsList);
            }
            else
            {
                cv.Projects = "[]";
