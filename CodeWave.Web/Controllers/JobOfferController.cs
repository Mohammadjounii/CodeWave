...
namespace CodeWave.Web.Controllers
{
    ...
    public class JobOfferController : Controller
    {
        ...
        private async Task<IActionResult> _OldIndex(string searchTerm = "", Guid? selectedJobId = null)
        {
            ...
            // Get active job offers filtered by user's level
            var allJobOffers = await _jobOfferService.GetActiveJobOffersAsync(searchTerm);
            var userLevel = user.Level ?? "Intermediate";
            var jobOffers = allJobOffers.Where(j =>
                j.RequiredLevel == "Both" ||
                j.RequiredLevel.Equals(userLevel, StringComparison.OrdinalIgnoreCase)
            ).ToList();
        ...
        public async Task<IActionResult> Apply(Guid jobId, string coverLetter = "")
        {
            ...
            if (_achSvc != null)
                await _achSvc.TryAwardAsync(userGuid, "first_job_app");
